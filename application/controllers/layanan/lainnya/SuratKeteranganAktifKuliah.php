<?php
class SuratKeteranganAktifKuliah extends CI_Controller {
    
        public function __construct()
        {
                parent::__construct();
                $this->load->model('surat_keterangan_aktif_kuliah_model');
        ...
        public function index()
        {
                if ($this->session->userdata('hak_akses') == 'Ketua Jurusan') {
                    ...
                    $data['title_bar'] = "Application";
                    $data['active'] = "Surat Keterangan Aktif Kuliah";
                    $data['page_title'] = "Surat Keterangan Aktif Kuliah";
                    $data['query'] = $this->surat_keterangan_aktif_kuliah_model->get_entries();
                    $data['activity'] = $this->log_aktifitas_model->get_where_entries();
                    $data['content'] = "layanan/lainnya/surat-keterangan-aktif-kuliah";
                    $data['this_page_plugin'] =
                    ...
                    $data['query'] = "";
                    $data['activity'] = $this->log_aktifitas_model->get_where_entries();
                    $data['content'] = "layanan/formulir-layanan/lainnya/surat-keterangan-aktif-kuliah";
                    $data['this_page_plugin'] =
                        '
                            <script type="text/javascript" src="' . base_url() . 'assets/js/plugins/icheck/icheck.min.js"></script>
                            <script type="text/javascript" src="' . base_url() . 'assets/js/plugins/mcustomscrollbar/jquery.mCustomScrollbar.min.js"></script>
        ...
        public function detail($id)
        {
                if ($this->session->userdata('hak_akses') == 'Pegawai') {
                    ...
                    $data['active'] = "Surat Keterangan Aktif Kuliah";
                    $data['page_title'] = "Surat Keterangan Aktif Kuliah";
                    $data['query'] = $this->surat_keterangan_aktif_kuliah_model->get_where_entries($id);
                    $data['activity'] = $this->log_aktifitas_model->get_where_entries();
                    $data['content'] = "layanan/lainnya/detail-surat-keterangan-aktif-kuliah";
                    $data['this_page_plugin'] =
                        '
                            <script type="text/javascript" src="' . base_url() . 'assets/js/plugins/icheck/icheck.min.js"></script>
                            <script type="text/javascript" src="' . base_url() . 'assets/js/plugins/mcustomscrollbar/jquery.mCustomScrollbar.min.js"></script>
        ...
        {
            if ($this->session->userdata('hak_akses') == 'Pegawai') {
                $query = $this->surat_keterangan_aktif_kuliah_model->get_where_entries($id);
                $this->load->library('PHPWord');
                $document = $this->phpword->loadTemplate('application/public/word/surat-keterangan-aktif-kuliah-template.docx');
        ...
        public function insert()
        {
                ...
                redirect(base_url() . 'layanan/lainnya/surat-keterangan-aktif-kuliah');
        }
        
        public function replace($id)
        {
                if ($this->session->userdata('hak_akses') == 'Pegawai') {
                    $this->surat_keterangan_aktif_kuliah_model->update_entry($id);
                    $this->session->set_flashdata('flash_data',
                        '
                        <div class="col-md-3"></div>
                        <div class="col-md-6">
                    ...
                    redirect(base_url() . 'pegawai/layanan/lainnya/surat-keterangan-aktif-kuliah/' . $id);
                } else {
                    redirect(base_url() . 'authentication');
                }
        }    
        ...
        {
                if ($this->session->userdata('hak_akses') == 'Pegawai') {
                    $this->surat_keterangan_aktif_kuliah_model->delete_entry($id);
                    $this->session->set_flashdata('flash_data',
                        '
                    ...
                    redirect(base_url() . 'pegawai/layanan/lainnya/surat-keterangan-aktif-kuliah');
                } else {
                    redirect(base_url() . 'authentication');
                }
        }
